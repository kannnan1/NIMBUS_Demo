...
# use create(*arg) to create dataset
# use create_model(*arg, *model_technique, *train_table, *target_column) to create model

# WARNING!!! DO NOT CHANGE FUNCTION DEFINITION
def MfactorCalculation(args):
    RatingTransition_Data=args[0]
    ...
    #f.save_table(avg_historicMatrix,name="avg_historic matrix")
    #f.create_df(avg_historicMatrix,preferred_name="avg_historic matrix")
    #print(np.arange(0,avg_historicMatrix.columns.shape[1]).astype("str"))
    ...
    tt = np.arange(0,Weight_Histrans_matrix.shape[0],9)
        ...
        # for k in np.arange(0,Weight_Histrans_matrix.shape[0],8):
        #     d=np.arange(0,estimated_matrix.shape[0],8)
        #     for i in d:
        #         aa1=estimated_matrix[i:(i+8),0:9] - Weight_Histrans_matrix[k:(k+8),0:9]
        #         if i==0:
        #             aa=aa1
        #         else:
        #             aa=aa
        ...
        #     difference_mat=aa
        #     if k==0:
        #         difference_mat1=difference_mat
        #     else:
        #         difference_mat1=np.hstack((difference_mat1,difference_mat))
        #     aa=[] 
        #     difference_mat = [] 
    ...
    print(summary_table)
    create(Mfactor)
    fig=px.scatter(Mfactor,x="Date", y ="Mfactor")
    fig.show()
